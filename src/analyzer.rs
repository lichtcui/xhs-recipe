...
pub async fn extract_recipe(
    ...
    let recipe_schema = json!({
        ...
        "properties": {
            ...
            "reason": {"type": "string"},
            "tags": {"type": "array", "items": {"type": "string"}, "description": "标签，如 [\"家常菜\", \"硬菜\", \"快手菜\"]"},
            "difficulty": {"type": "string", "enum": ["easy", "medium", "hard"], "description": "难度"},
            "servings": {"type": "integer", "description": "份量（几人份）"}
        },
...
8. **标签 (tags)**：菜系分类标签，如 ["家常菜", "硬菜", "快手菜", "川菜", "粤菜"]，根据菜谱特点推断
9. **难度 (difficulty)**：制作难度，取值为 "easy"（简单）、"medium"（中等）或 "hard"（困难）
10. **份量 (servings)**：菜谱适合几人份（整数）

注意事项：
- 如果内容与美食/菜谱无关，设置 is_food=false 并说明原因
- 如果某些信息在内容中没有明确提及，**不要编造**
- **【关键】必须完整提取每一个操作细节，包括预处理、腌制、焯水、爆香、翻炒、煎制、油炸、蒸煮等所有烹饪动作。视频 OCR 中的英文关键词（如 fried=煎/炒、boiled=煮、steamed=蒸、marinated=腌制）必须准确翻译并在步骤中体现**
- 用量单位保持原文（如克、毫升、勺、碗等）
...
fn parse_recipe_data(data: &Value) -> Recipe {
    ...
    Recipe {
        ...
        image_urls: None,
        tags: data["tags"].as_array().map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect()),
        difficulty: data.get("difficulty").and_then(|v| v.as_str()).map(String::from),
        servings: data.get("servings").and_then(|v| v.as_u64()).map(|n| n as u32),
        raw_text: None,
    }
}
