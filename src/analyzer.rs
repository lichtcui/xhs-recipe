...
pub async fn extract_recipe(
    ...
    let recipe_schema = json!({
        ...
        "properties": {
            ...
            "reason": {"type": "string"},
            "tags": {"type": "array", "items": {"type": "string"}, "description": "菜系分类标签，最多5个，如 [\"家常菜\", \"硬菜\", \"快手菜\"]"},
        },
...
8. **标签 (tags)**：输出 3-5 个最相关的菜系分类标签，根据菜谱特点推断；不要照搬原文中的话题标签 (#话题)
...
fn parse_recipe_data(data: &Value) -> Recipe {
    ...
    Recipe {
        ...
        image_urls: None,
        tags: data["tags"].as_array().map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).take(10).collect()),
        raw_text: None,
    }
}
